...
/// <summary>
/// A component that checks links for existence.
/// </summary>
public static class LinkChecker
{
    ...
    private static readonly HttpClientHandler clientHandler = new() { AllowAutoRedirect = true };
    private static readonly HttpClient client = new(clientHandler) { Timeout = HttpTimeout };
    ...
    public static List<LinkDescription> FindLinks(string html)
    {
        ...
        foreach (var link in links)
        {
            ...
            {
                LinkText = HttpUtility.HtmlDecode(link.InnerText),
                OriginalLink = link.GetAttributeValue("href", string.Empty)
            };
    ...
    /// <summary>
    /// Checks the link for existence and redirects.
    /// </summary>
    ...
    private static string CleanQueryString(string original, LinkQueryCleanLevel cleanLevel)
    {
        ...
        var qry = original[(original.IndexOf('?') + 1)..];
