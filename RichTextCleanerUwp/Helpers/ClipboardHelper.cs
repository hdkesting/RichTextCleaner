...
EndSelection:<<<<<<<<4";
        ...
        // https://docs.microsoft.com/en-us/windows/win32/dataxchg/standard-clipboard-formats doesn't help
        public const string FormatHtml = "HTML Format";
        ...
        public static DataPackage CreateDataObject(string html, string plainText)
        {
            ...
            dataObject.SetData(FormatHtml, htmlFragment);
        ...
        public async static Task<string> GetTextFromClipboardAsync()
        {
            var package = Clipboard.GetContent();
            Debug.WriteLine(package.AvailableFormats.Count);
            Debug.WriteLine(String.Join("; ", package.AvailableFormats));
            if (package.Contains(FormatHtml))
            {
                return GetHtmlFragment(await package.GetHtmlFormatAsync());

            }
            else if (package.Contains(FormatText))
            {
